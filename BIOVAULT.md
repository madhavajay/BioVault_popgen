...
```
ghcr.io/madhavajay/biovault-popgen:0.2.7-fast
```

The full image is still built as `ghcr.io/madhavajay/biovault-popgen:0.2.7` for slow/Hail
...
2. **FST** — `container ghcr.io/madhavajay/biovault-popgen:0.2.7-fast`. Load/merge
...
VCF, and aggregates per-gene PGx calls by `country`. The summary plots
treat non-reference / non-wildtype PharmCAT diplotypes as a PGx
call-burden signal, since PharmCAT report TSVs do not include population
allele frequencies.

**Input.** Same `List[GenotypeRecord]` samplesheet as the other flows,
plus the required `country` facet.
...
3. **Aggregation** — adds the `country` facet to every PharmCAT gene row,
   then writes participant-level and country-level count summaries.
4. **Visualization** — `04_population_level/pgx/plot_pgx_accumulation.py`
   writes a country heatmap showing where non-reference PGx calls
   accumulate by gene, plus a top-gene burden bar chart.

**Outputs**
- `pgx_participant_results.tsv` — one row per participant and PharmCAT
  gene result, with a `country` column.
- `pgx_participant_possible_genotypes.tsv` — one row per participant,
  country, gene, and unresolved possible genotype combination.
- `pgx_country_gene_genotype_counts.tsv` — counts grouped by country,
  gene, and possible genotype combination.
- `pgx_country_summary.tsv` — counts grouped by country, gene, diplotype,
  and phenotype.
...
- `pgx_gene_country_burden.tsv` — non-reference PGx call rate/count by
  country and gene.
- `pgx_plots/*` — the country heatmap and top-gene burden chart as
  PNG/PDF.
...
- `05_bv_paper_pgx`: `pgx_participant_results.tsv`,
  ...
  `pgx_country_gene_genotype_counts.tsv`, `pgx_country_summary.tsv`,
  `pgx_participant_manifest.tsv`, `pgx_gene_country_burden.tsv`,
  `pgx_plots/*`, `vcfs/*.vcf.gz`,
...
`ghcr.io/madhavajay/biovault-popgen:0.2.7-fast` bakes:
...
`ghcr.io/madhavajay/biovault-popgen:0.2.7` bakes:
...
```bash
VERSION=0.2.7 ./build_docker.sh
```

This builds the full image first, then the fast image. It produces:

```text
ghcr.io/madhavajay/biovault-popgen:0.2.7
...
docker build --platform linux/amd64 \
  --target fast-runtime \
  -t ghcr.io/madhavajay/biovault-popgen:0.2.7-fast \
...
```bash
docker tag biovault-popgen:fast ghcr.io/madhavajay/biovault-popgen:0.2.7-fast
IMAGE=ghcr.io/madhavajay/biovault-popgen:0.2.7-fast ./03_individual_level.sh --qc 3
IMAGE=ghcr.io/madhavajay/biovault-popgen:0.2.7-fast ./03_individual_level.sh --fast 3
IMAGE=ghcr.io/madhavajay/biovault-popgen:0.2.7-fast ./03_individual_level.sh --sex 4
IMAGE=ghcr.io/madhavajay/biovault-popgen:0.2.7-fast ./04_population_level.sh --limit 1
```
...
```bash
IMAGE=ghcr.io/madhavajay/biovault-popgen:0.2.7 ./03_individual_level.sh --slow 3
IMAGE=ghcr.io/madhavajay/biovault-popgen:0.2.7 ./03_individual_level.sh --qc --slow 3
IMAGE=ghcr.io/madhavajay/biovault-popgen:0.2.7 ./03_individual_level.sh --sex --slow 4
IMAGE=ghcr.io/madhavajay/biovault-popgen:0.2.7 ./04_population_level.sh --slow --limit 1
```
...
| Flow                                       | Step | Image |
|--------------------------------------------|------|-------|
| `01_bv_paper_pca_qc_fast`                  | 1    | `ghcr.io/madhavajay/biovault-popgen:0.2.7-fast` |
| `02_bv_paper_gnomad_projection_fast`       | 2    | `ghcr.io/madhavajay/biovault-popgen:0.2.7-fast` |
| `03_bv_paper_sex_biased_admixture_fast`    | 3    | `ghcr.io/madhavajay/biovault-popgen:0.2.7-fast` |
| `04_bv_paper_population_level`             | 4    | `ghcr.io/openmined/biosynth:0.1.31` + `ghcr.io/madhavajay/biovault-popgen:0.2.7-fast` |
| `05_bv_paper_pgx`                          | 5    | `ghcr.io/openmined/biosynth:0.1.31` + `pgkb/pharmcat` + `ghcr.io/madhavajay/biovault-popgen:0.2.7-fast` |
