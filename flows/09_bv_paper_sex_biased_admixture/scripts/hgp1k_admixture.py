...
(AFR/EUR/SAS, baked as PLINK BED) at K = 5, run on:
...
X handling: males are hemizygous, so the study `sex` facet is materialised and
applied (`plink2 --update-sex`) before the X run. ADMIXTURE is called with
`--haploid=male:23` for X-bearing runs, so male non-PAR X is modelled as
haploid. All samples are kept; a per-sex breakdown of the X estimates is
emitted.
...
_X_MIND_RAW = os.environ.get("BV_X_MIND", "0.999").strip().lower()
X_MIND = None if _X_MIND_RAW in {"", "none", "off", "skip"} else float(_X_MIND_RAW)
...
def fam_sex_counts(prefix: Path, keep_ids: set[str] | None = None) -> str:
    ...
    for line in Path(f"{prefix}.fam").open():
        ...
        if keep_ids is not None and iid not in keep_ids:
            continue
        sex = parts[4] if len(parts) > 4 else "other"
        counts[sex if sex in counts else "other"] += 1
    return f"male={counts['1']} female={counts['2']} unknown={counts['0']} other={counts['other']}"
...
def qc_and_prune(merged: Path, region: str, work: Path, plink2, threads):
    ...
    qc_cmd = [plink2, "--bfile", merged]
    if region == "x":
        qc_cmd += ["--geno", GENO, "hh-missing"]
        if X_MIND is not None:
            qc_cmd += ["--mind", X_MIND, "hh-missing"]
    elif region == "combined":
        qc_cmd += ["--geno", GENO, "hh-missing", "--mind", MIND, "hh-missing"]
    else:
        qc_cmd += ["--geno", GENO, "--mind", MIND]
    qc_cmd += ["--maf", MAF, "--hwe", HWE, "--make-bed", "--out", qc,
               "--allow-no-sex", "--threads", threads, "--memory", "6000"]
    if region in {"x", "combined"}:
        qc_cmd.append("--set-invalid-haploid-missing")
    run(qc_cmd, log=work / f"qc_{region}.log")
...
# --- ADMIXTURE --------------------------------------------------------------
def admixture_chrom(chrom: str) -> str:
    chrom = str(chrom).replace("chr", "")
    if chrom in {"X", "23"}:
        return "23"
    if chrom in {"Y", "24"}:
        return "24"
    if chrom in {"PAR1", "PAR2", "XY", "25"}:
        return "25"
...
def admixture_ready(pruned: Path, work: Path, tag: str) -> Path:
    """Copy PLINK files and normalize chromosome names for ADMIXTURE.

    ADMIXTURE requires numeric chromosome codes. We preserve autosomes as 1-22
    and map chrX to 23 so --haploid=male:23 applies to X-bearing runs.
    """
    dst = work / f"adx_{tag}"
...
def run_admixture(pruned: Path, tag: str, k: int, work: Path, out_dir: Path,
    ...
    log_msg(f"step 6/8: running ADMIXTURE for {tag} K={k}")
    adx = admixture_ready(pruned, work, f"{tag}_K{k}")
    seed = SEED + k
    cmd = [admixture, "-s", seed, "-j" + str(threads), f"{adx}.bed", k]
    if tag in {"x", "combined"}:
        cmd.append("--haploid=male:23")
    run(cmd, cwd=work, log=out_dir / f"admixture_{tag}_K{k}.log")
...
def main():
    ...
    ap.add_argument("--k-values", default=os.environ.get("BV_ADMIXTURE_K", "5"))
    ...
    for tag, pref in runs.items():
        for k in ks:
            ...
            log_msg(f"step 7/8: labelling ADMIXTURE components for {tag} K={k}")
    ...
    (out / "qc_report.txt").write_text(
        ...
        f"K values: {ks}\n"
        f"reference: {args.reference_dir}\n"
        f"geno={GENO} mind={MIND} x_mind={X_MIND if X_MIND is not None else 'off'} "
        f"maf={MAF} hwe={HWE} "
        f"ld=({LD_WINDOW},{LD_STEP},{LD_R2})\n"
        + "\n".join(qc_lines) + "\n"
        "NOTE: ADMIXTURE is run with --haploid=male:23 for X-bearing runs; "
        "X estimates include a per-sex breakdown "
        "(mean_x_female / mean_x_male_haploid) for that reason.\n")
