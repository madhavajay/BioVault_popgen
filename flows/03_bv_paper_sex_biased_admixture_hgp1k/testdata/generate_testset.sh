...
#   IMAGE=ghcr.io/madhavajay/biovault-admixture:latest
#
...
N="${2:-100}"
IMAGE="${IMAGE:-ghcr.io/madhavajay/biovault-admixture:latest}"
...
docker run --rm --platform linux/amd64 -u "$(id -u):$(id -g)" \
  -v "${ROOT_DIR}:/work" -v "${OUT_DIR}:/out" --entrypoint bash "${IMAGE}" -lc "
