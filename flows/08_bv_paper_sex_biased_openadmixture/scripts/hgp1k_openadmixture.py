#!/usr/bin/env python3
"""HGP1K-anchored sex-biased OpenADMIXTURE.

Joint OpenADMIXTURE of the study cohort + a frozen 1000 Genomes reference
...
Components are anonymous out of OpenADMIXTURE, so each is LABELLED by which
...
Pipeline (all validated against real 1KGP + bvs output):
  study_raw (bvs cohort-bed, mixed rsID/chr:pos IDs, monomorphic rows)
    ...
    -> OpenADMIXTURE K on combined / auto / x
...
SEED = int(os.environ.get("BV_OPENADMIXTURE_SEED", os.environ.get("BV_ADMIXTURE_SEED", "1729")))
...
# --- OpenADMIXTURE ----------------------------------------------------------
def openadmixture_ready(pruned: Path, work: Path, tag: str) -> Path:
    """Copy bed/fam, rewrite .bim chromosome col to '1' for parity with the
    original ADMIXTURE flow and to avoid non-integer chromosome codes."""
    dst = work / f"adx_{tag}"
...
def run_openadmixture(pruned: Path, tag: str, k: int, work: Path, out_dir: Path,
    ...
    run([openadmixture, "--bed", f"{adx}.bed", "--k", k,
         "--out-prefix", f"{adx}.{k}", "--seed", seed, "--threads", threads],
        cwd=work, log=out_dir / f"openadmixture_{tag}_K{k}.log")
    q_src = work / f"{adx.name}.{k}.Q"
    p_src = work / f"{adx.name}.{k}.P"
    q_dst = out_dir / f"openadmixture_{tag}_K{k}.Q"
    p_dst = out_dir / f"openadmixture_{tag}_K{k}.P"
...
def plot_figure(per_sample: pd.DataFrame, cohort: pd.DataFrame, k: int,
    ...
    fig.suptitle(f"HGP1K sex-biased OpenADMIXTURE - K={k} (focal ancestry: {focal})", fontsize=12)
...
def main():
    ...
    ap.add_argument("--k-values", default=os.environ.get("BV_OPENADMIXTURE_K", os.environ.get("BV_ADMIXTURE_K", "5")))
    ap.add_argument("--threads", default=os.environ.get("BV_THREADS", "8"))
    args = ap.parse_args()

    plink, plink2, openadmixture = tool("plink"), tool("plink2"), tool("openadmixture-bed")
    ...
    (out / "qc_report.txt").write_text(
        "=== hgp1k sex-biased OpenADMIXTURE QC ===\n"
        f"K values: {ks}\n"
